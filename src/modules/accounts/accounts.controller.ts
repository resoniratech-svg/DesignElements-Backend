    ...
    try {
        ...
        const receivablesRes = await pool.query(
            `SELECT COALESCE(SUM(balance_amount), 0) as total FROM invoices 
             WHERE status != 'PAID' AND approval_status = 'approved' AND deleted_at IS NULL${divisionFilter}`,
            params
        );
        ...
        const pendingRes = await pool.query(
            `SELECT COALESCE(SUM(total_amount), 0) as total FROM invoices 
             WHERE approval_status = 'pending' AND deleted_at IS NULL${divisionFilter}`,
            params
        );
