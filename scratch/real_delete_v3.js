...
async function realDelete() {
  ...
  try {
    ...
    console.log('Deleted client record:', deleteRes.rows[0].id);

    if (userId) {
      // Mapped to users.id
      console.log('Cleaning up user-linked invoices/projects...');
      ...
      console.log('Deleted associated user:', userDeleteRes.rows[0].id);
    }
